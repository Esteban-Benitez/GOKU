...
print("="*40)
np.save("goku_pred.npy", goku_pred.cpu().numpy() if torch.is_tensor(goku_pred) else goku_pred)
np.save("latent_pred.npy", latent_pred.cpu().numpy() if torch.is_tensor(latent_pred) else latent_pred)
np.save("lstm_pred.npy", lstm_pred.cpu().numpy() if torch.is_tensor(lstm_pred) else lstm_pred)
np.save("x_extrap_target.npy", x_extrap_target.cpu().numpy() if torch.is_tensor(x_extrap_target) else x_extrap_target)