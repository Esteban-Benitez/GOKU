...
def train(args):
    ...
    val_dataloader = torch.utils.data.DataLoader(ds_val, batch_size=len(ds_val), shuffle=False)

    # Create model
    model = models.__dict__["create_latent_ode_" + args.model](ode_method=args.method).to(device)
    print('Model: Latent ODE - %s created with %d parameters.' % (args.model, sum(p.numel() for p in model.parameters())))
    ...
    for epoch in range(args.num_epochs):
        ...
        for i_batch, mini_batch in enumerate(train_dataloader):
            ...
            pred_x, pred_z, z_0, z_0_loc, z_0_log_var = model(mini_batch, t=t, variational=True)
        ...
        val_loss = validate_latent_ode(args, model, val_dataloader, device)
