...
print("-"*45)
paper_numbers = {
    'GOKU':      0.4,  # Replace with actual paper values
    'Latent-ODE':2.8,
    'LSTM':      5.3,
    'DMM':       4.5,
    'DI 1%':     41.9,
...
# --- Safe .npy Saving ---
np.save("goku_pred_cvs.npy", goku_pred.cpu().numpy() if torch.is_tensor(goku_pred) else goku_pred)
if latent_pred is not None:
    np.save("latent_pred_cvs.npy", latent_pred.cpu().numpy() if torch.is_tensor(latent_pred) else latent_pred)
else:
    print("WARNING: latent_pred was not computed, skipping save.")

if lstm_pred is not None:
    np.save("lstm_pred_cvs.npy", lstm_pred.cpu().numpy() if torch.is_tensor(lstm_pred) else lstm_pred)
else:
    print("WARNING: lstm_pred was not computed, skipping save.")

np.save("x_extrap_target_cvs.npy", x_extrap_target.cpu().numpy() if torch.is_tensor(x_extrap_target) else x_extrap_target)

# =================== Plot curves ===================
try:
    goku_pred_np = np.load("goku_pred_cvs.npy")
    latent_pred_np = np.load("latent_pred_cvs.npy")
    lstm_pred_np = np.load("lstm_pred_cvs.npy")
    x_extrap_target_np = np.load("x_extrap_target_cvs.npy")
    ...
    plt.tight_layout()
    plt.savefig("cvs_extrapolation_error.png")
    plt.show()
except Exception as e:
