...
goku_ckpt = load_checkpoint("checkpoints/double_pendulum/goku_model.pkl", device=device)
latent_ode_ckpt = load_checkpoint("checkpoints/double_pendulum/latent_ode_model.pkl", device=device)
lstm_ckpt = load_checkpoint("checkpoints/double_pendulum/lstm_model.pkl", device=device)
...
np.save("goku_pred_doublependulum.npy", goku_pred.cpu().numpy() if torch.is_tensor(goku_pred) else goku_pred)
np.save("latent_pred_doublependulum.npy", latent_pred.cpu().numpy() if torch.is_tensor(latent_pred) else latent_pred)
np.save("lstm_pred_doublependulum.npy", lstm_pred.cpu().numpy() if torch.is_tensor(lstm_pred) else lstm_pred)
np.save("x_extrap_target_doublependulum.npy", x_extrap_target.cpu().numpy() if torch.is_tensor(x_extrap_target) else x_extrap_target)

# =================== Plot curves ===================
goku_pred = np.load("goku_pred_doublependulum.npy")           # shape [batch, 50, 28, 28]
latent_pred = np.load("latent_pred_doublependulum.npy")
lstm_pred = np.load("lstm_pred_doublependulum.npy")
x_extrap_target = np.load("x_extrap_target_doublependulum.npy")
...
plt.tight_layout()
plt.savefig("doublependulum_extrapolation_error.png")
plt.show()